...
const WaveformSDF = (): JSX.Element => {
  ...
  const glContextRef = useRef<WebGL2RenderingContext>();
  const speedUniformRef = useRef<WebGLUniformLocation>();

  const { isPlaying } = usePlaylist();
  useEffect(() => {
    console.log(isPlaying);
    if (glContextRef.current && speedUniformRef.current) {
      if (isPlaying) {
        glContextRef.current.uniform1f(speedUniformRef.current, 0.1);
      } else {
        glContextRef.current.uniform1f(speedUniformRef.current, 0.0);
      }
    }
  }, [isPlaying]);
  ...
  useEffect(() => {
    ...
    if (canvasRef.current && wrapperRef.current) {
      ...
      const { iResolution, iTime, matCap, cloudNoise, canvas, Speed } =
...
type UniformResults = { [n: string]: WebGLUniformLocation };
const getUniforms = (
  gl: WebGL2RenderingContext,
